...
public class Presentation extends Applet implements KeyListener, Runnable, MouseListener, MouseMotionListener, ActionListener
{
    ...
    public double b = 0;
    public double GRAVITY = 0.00981, MASS = 1, BOUNCE = 0.8, COF = 0.1, FRICTION = COF * (MASS * (GRAVITY * 1000)); //move when menu is made
    ...
    public boolean moveableX = true, moveableY = true, throwable = false;
    ...
    public void paint (Graphics page)
    {
        ...
        buffer.fillRect(0, screenY, screenX, screenY + 50);
        ...
        switch (mode)
        {
            case MENU:
            {
                ...
                gravity = new TextField ((fmt.format(GRAVITY * 1000)), 4);
                mass = new TextField ((fmt.format(MASS)), 4);
                bounce = new TextField ((fmt.format((BOUNCE * 100))), 4);
                cof = new TextField ((fmt.format(COF)), 4);
                ballSize = new TextField ((fmt.format(((double)width / 100))), 4);
                ...
                gravityLabel.setBounds(25, screenY, 100, 25);
                gravity.setBounds(125, screenY + 5, 40, 25);
                massLabel.setBounds(175, screenY, 100, 25);
                mass.setBounds(275, screenY + 5, 40, 25);
                bounceLabel.setBounds(325, screenY, 150, 25);
                bounce.setBounds(475, screenY + 5, 40, 25);
                cofLabel.setBounds(525, screenY, 150, 25);
                cof.setBounds(675, screenY + 5, 40, 25);
                ballSizeLabel.setBounds(725, screenY, 150, 25);
                ballSize.setBounds(875, screenY + 5, 40, 25);
                update.setBounds(1050, screenY + 5, 100, 30);
            ...
            case MOVE:
            {
                try
                {
                    ...
                    if ((ballYRight >= screenY || ballYLeft >= screenY) && up == false && !throwable)   //bottom
                    {
                         keY *= BOUNCE;
                         velocityY = (Math.sqrt(keY / (0.5 * MASS)));
                         velocityY *= -1;
                    }
                    else if ((ballYRight >= (screenY + (height / 2)) || ballYLeft >= (screenY + (height / 2))) && throwable)
                    {
                        ballYRight = height - (height / 2);
                        ballYLeft = 0 - (height / 2);
                        image++;
                        throwable = false;
                    }
                        
                    if ((ballYLeft <= 0 || ballYRight <= 0) && up == true && !throwable)  //top
                    {
                        keY *= BOUNCE;
                        velocityY = (Math.sqrt(keY / (0.5 * MASS)));
                    }
                    else if ((ballYRight <= (0 - (height / 2)) || ballYLeft <= (0 - (height / 2))) && throwable)
                    {
                        ballYRight = screenY + (height / 2);
                        ballYLeft = screenY - (height / 2);
                        image--;
                        throwable = false;
                    }
                    
                    if ((ballXLeft <= 0 || ballXRight <= 0) && left == true && !throwable)    //left
                    {
                        keX *= BOUNCE;
                        velocityX = (Math.sqrt(keX / (0.5 * MASS)));
                    }
                    else if ((ballXRight <= (0 - (width / 2)) || ballXLeft <= (0 - (width / 2))) && throwable)
                    {
                        ballXRight = screenX + (width / 2);
                        ballXLeft = screenX - (width / 2);
                        image--;
                        throwable = false;
                    }
                    
                    if ((ballXLeft >= screenX || ballXRight >= screenX) && left == false && !throwable)   //right
                    {
                        keX *= BOUNCE;
                        velocityX = (Math.sqrt(keX / (0.5 * MASS)));
                        velocityX *= -1;
                    }
                    else if ((ballXRight >= (screenX + (width / 2)) || ballYLeft >= (screenY + (width / 2))) && throwable)
                    {
                        ballXRight = width - (width / 2);
                        ballXLeft = 0 - (width / 2);
                        image++;
                        throwable = false;
                    }
                    ...
                    if ((ballYLeft == (screenY - 50) || ballYRight == screenY) && left == true && velocityX < 0.0000 && moveableX == true && !throwable)  //FRICTION left
                        {
                    ...
                    if ((ballYLeft == (screenY - 50) || ballYRight == screenY) && left == false && velocityX > 0.0000 && moveableX == true && !throwable)    //FRICTION right
                    {
